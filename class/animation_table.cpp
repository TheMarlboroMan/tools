...
const animation_line& animation::get_for_time(float t) const
{
	...
	{
		float transformado=fmod(t, duration);
		for(const animation_line& fr : data)
		{
			if(transformado <= fr.begin_time) return fr;
		}
		return data.at(0);
	}
}
...
const animation_line& animation::get_for_time(float t, float total) const
{
	...
	else
	{
		...
		for(const animation_line& fr : data)
		{
			if(transformado <= fr.begin_time * mult) return fr;
		}
		return data.at(0);
	}
}

//Aquí definimos los métodos de la tabla de data.
